...
pub trait StateMachine<C> {
    ...
    fn postcondition(&self, cmd: C, res: &(dyn Any)) -> bool;
...
pub struct CommandSequenceValueTree<T> {
    elements: Vec<Box<dyn ValueTree<Value = T>>>,
    num_included: usize,
    shrink: Shrink,
    prev_shrink: Option<Shrink>,
}

impl<T: Debug> ValueTree for CommandSequenceValueTree<T> {
    type Value = Vec<T>;
